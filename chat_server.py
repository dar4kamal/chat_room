...
def clientthread(conn, addr):
    ...
    name = conn.recv(2048)
    clients.update({conn:name.decode().split('\n')[0],"addr":addr,"name":name.decode()})
    #sends a message to the client whose user object is conn
    while True:
            try:     
                message = conn.recv(2048) 
                #print("recv from {} > {}".format(addr[1],message))   
                message = message.decode()
                if message:

                    print("<" + clients[conn] + "> " + message)
                    message_to_send = "<" + clients[conn] + "> " + message
